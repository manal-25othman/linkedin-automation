...
- الاختبارات: `tests/sql/*` (٢٥٧ حالة) + `90_mutation.sql` (يجب أن **تفشل** ٥٢ حالة عند تعطيل الحرّاس) + `tests/unit/*` (٦٩٧ ناجحًا).
...
2. شغّل `npm test` و`tests/sql/run-isolation-tests.sh`؛ يجب أن تبقى الأرقام: وحدة ٦٩٧ ناجحًا | SQL ٢٥٧/٢٥٧ | طفرات ٥٢ فاشلة.
