...
- الاختبارات: `tests/sql/*` (٢٧١ حالة) + `90_mutation.sql` (يجب أن **تفشل** ٥٩ حالة عند تعطيل الحرّاس) + `tests/unit/*` (٧٢٨ ناجحًا).
...
2. شغّل `npm test` و`tests/sql/run-isolation-tests.sh`؛ يجب أن تبقى الأرقام: وحدة ٧٢٨ ناجحًا | SQL ٢٧١/٢٧١ | طفرات ٥٩ فاشلة.
