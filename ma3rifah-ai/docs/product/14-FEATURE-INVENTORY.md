...
**مصدر الجرد:** الفرع `main` — الهجرة الأخيرة `0037_feedback_survey.sql` —
٤٩ إجراء خادم — ١٩ صلاحية — ٣٧ جدولًا — ٦٤ ملف اختبار وحدة + ١٠ ملفات اختبار SQL.
...
2. شغّل `npm test` و`tests/sql/run-isolation-tests.sh`؛ يجب أن تبقى الأرقام: وحدة ٦٨٢ ناجحًا | SQL ٢٣٢/٢٣٢ | طفرات ٣٧ فاشلة.
3. قارن قائمة الإجراءات: `grep -rhoE "export async function \w+Action" src/app | sort` يجب أن تُعطي ٤٩ إجراءً نفسها.
4. قارن المسارات: `find src/app -name page.tsx -o -name route.ts` يجب ألا يقل عن ٥٣ ملفًا.
