# دليل الضبط — خطوة بخطوة

دليل تشغيلي لضبط المنصة على بيئة حقيقية. مرتّب بحيث لا تحتاج خطوةٌ ما بعدها،
فيمكن التوقف عند أي نقطة والعودة لاحقًا.

الوقت التقريبي: ٤٥ دقيقة، عدا ما ينتظر جهة خارجية (السجل التجاري، توثيق النطاق).

> **اقرأ هذا أولًا — لا تُعِد ما هو مضبوط.**
> الدليل مكتوب للتثبيت من الصفر، وبعض خطواته قد تكون منجزة عندك منذ
> إعداد سابق. لكل خطوة **فحصٌ سريع** يقول لك: هل تحتاجها أصلًا؟ ابدأ
> بالفحص، ولا تُعِد الضبط إلا إن سقط.
>
> وإعادة الضبط بلا حاجة ليست ضارة، لكنها تُهدر وقتًا وتزرع شكًّا في أن
> شيئًا انكسر — وهو ما لم يحدث.

---

## ١) قاعدة البيانات — ترحيلات SQL

**المكان:** Supabase ← مشروعك ← SQL Editor ← New query

**ما تفعلينه:** افتحي ملف `supabase/ALL_MIGRATIONS.sql` من المستودع، انسخيه
كاملًا، والصقيه، ثم **Run**.

الملف آمن للتكرار: تشغيله مرة أو ثلاثًا يعطي النتيجة نفسها. مُختبَر على
PostgreSQL 16 من قاعدة جديدة ومن قاعدة قائمة، وفي الحالتين ثلاث جولات
متتالية بلا خطأ.

**للتحقق أنه نجح** — الصقي هذا وشغّليه:

```sql
select
  (select count(*) from pg_proc where proname = 'check_user_quota')                   as "حد المستخدمين",
  (select count(*) from pg_proc where proname = 'check_storage_quota')                as "حد التخزين",
  (select count(*) from pg_proc where proname = 'activate_subscription_for_payment')  as "تفعيل الاشتراك",
  (select count(*) from pg_tables where tablename = 'payments')                       as "جدول المدفوعات",
  (select case when indpred is null then 'كامل ✓' else 'جزئي ✗' end
     from pg_index i join pg_class c on c.oid = i.indexrelid
    where c.relname = 'notifications_unique_event_idx')                               as "فهرس التنبيهات",
  (select count(*) from pg_tables where tablename = 'site_content')                   as "محتوى الموقع",
  (select count(*) from pg_tables where tablename = 'site_pages')                     as "صفحات الموقع";
```

المتوقّع: كل عمود يعطي `1` عدا «فهرس التنبيهات» فيعطي «كامل ✓».

> **إن كنتِ شغّلتِ الملف قبلُ:** شغّليه ثانيةً. أُضيف بعده ترحيلان —
> `0023` يوسّع حدّ طول المحتوى المحرَّر، و`0024` ينشئ جدول الصفحات
> المصنوعة — وبدونهما لا يعمل محرِّر القوائم ولا صفحة «الصفحات».

المطلوب: أربعة أرقام قيمتها ١، وفهرس التنبيهات **كامل**.

- ظهرت كلها كذلك ⇒ **الخطوة منجزة، تجاوزها.**
- ظهر صفر في أيٍّ منها، أو «جزئي» ⇒ شغّل الملف. و«جزئي» تحديدًا تعني أن
  **لا تنبيه يصل إطلاقًا**.

وشغّل هذا الفحص **قبل** لصق الملف لا بعده: إن كان كل شيء مضبوطًا فلا
حاجة إلى شيء.

---

## ٢) روابط الدخول والبريد

**Supabase ← Authentication ← URL Configuration**

| الحقل | القيمة |
|---|---|
| Site URL | `https://ma3rifah-ai.vercel.app` |
| Redirect URLs | `https://ma3rifah-ai.vercel.app/auth/callback` |

بدون سطر `Redirect URLs` تقول كل روابط البريد «انتهت الصلاحية» وهي جديدة.

**Vercel ← Settings ← Environment Variables**

```
NEXT_PUBLIC_APP_URL=https://ma3rifah-ai.vercel.app
```

> أي تغيير في متغيرات Vercel يحتاج **Redeploy** ليسري. التغيير وحده لا يكفي.

**فحص سريع — هل تحتاج هذه الخطوة؟**

اطلب رابط استعادة كلمة مرور من `/forgot-password` وافتحه.

- فُتحت صفحة تعيين كلمة المرور ⇒ **الخطوة منجزة، تجاوزها.**
- ظهر «انتهت صلاحية الرابط» على رابط جديد ⇒ أحد السطرين ناقص.

هذا العَرَض هو أول ما ينكسر عند نقصهما، فصلاحه دليلٌ كافٍ على ضبطهما.

---

## ٣) البريد — إخراج الرسائل من مجلد المزعج

الرسائل الآن تُرسَل من نطاق مشترك، فتصل في «المزعج». الحل الدائم نطاق موثَّق:

1. Resend ← Domains ← Add Domain ← أدخلي نطاقك
2. أضيفي سجلات DNS التي تعرضها (SPF و DKIM) عند مزوّد نطاقك
3. انتظري التوثيق (دقائق إلى ساعات)
4. Supabase ← Authentication ← Emails ← SMTP Settings ← غيّري
   `Sender email` إلى بريد على نطاقك الموثَّق

**اختياري لكنه مفيد:** قوالب البريد في Supabase إنجليزية افتراضيًا. من
Authentication ← Email Templates يمكن تعريبها — خصوصًا «Reset Password»،
لأنها القالب الذي يصل الموظف الجديد ليضع كلمة مروره، و«Reset your password»
عنوانٌ مربك لمن لم يكن له حساب أصلًا.

---

## ٤) بوابة الدفع — Moyasar

### أ) الحساب والمفاتيح

1. سجّلي في [moyasar.com](https://moyasar.com)
2. Settings ← API Keys — ستجدين زوجين: **Test** و **Live**
3. ابدئي بمفاتيح **Test**: تعمل ببطاقات وهمية ولا تحتاج سجلًا تجاريًا

### ب) متغيرات Vercel

```
MOYASAR_SECRET_KEY=sk_test_xxxxxxxxxxxx
NEXT_PUBLIC_MOYASAR_PUBLISHABLE_KEY=pk_test_xxxxxxxxxxxx
MOYASAR_WEBHOOK_SECRET=<نصّ عشوائي طويل تختارينه أنتِ>
```

> ⚠ المفتاح السري (`sk_`) لا يوضع أبدًا في متغير يبدأ بـ`NEXT_PUBLIC_` —
> فذلك يرسله إلى متصفح كل زائر.

`MOYASAR_WEBHOOK_SECRET` قيمة تخترعينها أنتِ (٣٢ حرفًا عشوائيًا مثلًا)،
وتكتبينها في موضعين: هنا، وفي لوحة Moyasar بالخطوة التالية. لا بد أن
تتطابقا حرفًا بحرف.

### ج) الـwebhook

Moyasar ← Webhooks ← Add:

| الحقل | القيمة |
|---|---|
| URL | `https://ma3rifah-ai.vercel.app/api/webhooks/moyasar` |
| Events | `payment_paid` و `payment_failed` |
| Secret token | القيمة نفسها من `MOYASAR_WEBHOOK_SECRET` |

ثم **Redeploy** في Vercel.

### د) التجربة

الاشتراك والفواتير ← اختاري خطة ← «الاشتراك في هذه الخطة» ← ادفعي ببطاقة
الاختبار التي تعرضها Moyasar في وثائقها.

المتوقع: تعودين إلى الصفحة برسالة «تم استلام الدفعة وتفعيل اشتراكك»، وتظهر
الدفعة في «سجلّ المدفوعات»، وتتغيّر الخطة الحالية.

**المنصة تعمل بلا هذه المفاتيح.** تظهر رسالة أن البوابة غير مفعّلة، ولا
يتعطّل شيء آخر — فلا داعي لتأجيل بقية الخطوات.

---

## ٤٫٥) أمن الجلسات — بعد النشر مباشرة

بعد نشر هذه النسخة **ستُطالَبين أنتِ وكل من كان داخلًا بتسجيل الدخول مرة
واحدة**. هذا مقصود لا عطل: الجلسات القديمة أُنشئت قبل وجود سياسة عمر،
ولا سبيل إلى معرفة متى بدأت — فتُعامَل معاملة المنتهية.

**ما صار يحدث الآن:**

| الدور | تنتهي بالسكون بعد | وتنتهي في كل الأحوال بعد |
|---|---|---|
| مالك المنصة · مدير الشركة | **ساعة** | **12 ساعة** |
| مدير القسم · الموظف | **8 ساعات** | **30 يومًا** |

**متغيّر بيئة اختياري** (Vercel ← Settings ← Environment Variables):

```
SESSION_SECRET=<ولّده بـ: openssl rand -base64 32>
```

بدونه يُشتقّ السرّ من `SUPABASE_SERVICE_ROLE_KEY` تلقائيًا فيعمل كل شيء.
لكن ضبطه صراحةً أفضل: تدوير مفتاح الخدمة عندها لا يُخرج كل المستخدمين
دفعة واحدة.

**تحققي بنفسك:** ادخلي بحسابك، ثم افتحي **الإعدادات ← الملف الشخصي** —
ستجدين بطاقة «أمان الجلسة» تقول لكِ مدّتك، وفيها زرّ «تسجيل الخروج من
جميع الأجهزة».

---

## ٤٫٦) الأسعار والحدود الجديدة

شغّلي `ALL_MIGRATIONS.sql` مرة أخرى — أُضيف ترحيلان:

- **0025** يعيد تسعير الخطط ويضيف خطة **Growth** الوسطى
- **0026** يفعّل تنبيه «اقترب حدّ الأسئلة» عند ٨٠٪

| الخطة | السعر | مستخدمون | مستندات | أسئلة/شهر |
|---|---|---|---|---|
| Starter | ٨٩٩ ريال | ١٠ | ٥٠ | ٦٠٠ |
| **Growth** ⭐ | **٢٬٤٩٩ ريال** | **٣٠** | **٢٠٠** | **٢٬٠٠٠** |
| Business | ٥٬٩٩٩ ريال | ٧٥ | ٦٠٠ | ٦٬٠٠٠ |

وكلها قابلة للتعديل من **لوحة المنصة ← الخطط** بلا مطوّر.

> **لماذا تغيّرت:** التسعير الأول كان يخسر على كل عميل — التفصيل في
> `docs/product/04-PRICING-STRATEGY.md`.

**وتغيّر النموذج الافتراضي** من `claude-opus-5` إلى `claude-sonnet-5`،
والجهد إلى `low`، وسقف الخرج إلى ٣٠٠٠. لا يلزمك فعل شيء — تعمل تلقائيًا.
ولو أردتِ العودة إلى opus فاضبطي `ANTHROPIC_MODEL` في Vercel، **وراجعي
الحصص أولًا**.

**اختبري بنفسك بعد النشر:** اسألي المساعد عشرين سؤالًا حقيقيًا وقارني
جودة الإجابة بما كانت. إن نقصت، أعيدي `ANTHROPIC_EFFORT=medium` أولًا
(الهامش يبقى موجبًا)، ثم النموذج إن لزم.

---

## ٥) الصفحتان القانونيتان

`/privacy` و `/terms` جاهزتان نصًّا، وتعرضان تنبيهًا ظاهرًا أنهما مسوّدة حتى
تُضبط بيانات الكيان النظامي:

```
NEXT_PUBLIC_LEGAL_NAME=الاسم النظامي كما في السجل التجاري
NEXT_PUBLIC_LEGAL_CR=رقم السجل التجاري
NEXT_PUBLIC_LEGAL_VAT=الرقم الضريبي
NEXT_PUBLIC_LEGAL_ADDRESS=العنوان الوطني
NEXT_PUBLIC_LEGAL_EMAIL=بريد التواصل النظامي
```

النصّ صياغة تقنية لما تفعله المنصة فعلًا، **وليس استشارة قانونية**. اعرضيه
على مختص نظامي قبل أول عميل يدفع.

---

## ٥٫٥) فحص الدخان — نصف ساعة تسبق أي عميل

**لماذا هذا القسم:** كل عطل جسيم ظهر حتى الآن كان من نوع واحد: يعمل
محليًا ويفشل على الإنتاج. حدّ حجم الطلب على المنصة، وفهرس لا يصلح
مُحكِّمًا، ومسار مصادقة مفقود، ودالة تُستدعى بمفتاح خاطئ — لا يكشف
أيًّا منها بناءٌ ناجح ولا اختبار وحدة. يكشفها **الاستعمال** وحده.

فهذه الرحلة تمرّ بكل ما انكسر سابقًا. شغّليها **على الإنتاج** بعد أي
نشر مهمّ، وبحسابات حقيقية لا تجريبية.

| # | الخطوة | الدليل على النجاح | إن فشلت |
|---|---|---|---|
| ١ | سجّلي شركة جديدة بحساب مدير | وصلتِ لوحة التحكم | التسجيل أو تجهيز الشركة |
| ٢ | أضيفي موظفًا | وصلته رسالة، ووضع كلمة مروره بنفسه، ودخل | البريد أو `/auth/callback` |
| ٣ | **ارفعي ملفًا أكبر من ٥ ميغابايت** | صار «جاهز للسؤال» | حدّ حجم الطلب — أخطر ما تكرّر |
| ٤ | ارفعي ملفًا باسم عربي | نجح، والاسم العربي ظاهر في القائمة | مفتاح التخزين |
| ٥ | اسألي المساعد سؤالًا يجيب عنه المستند | إجابة صحيحة **مع مصدر ورقم صفحة** | الاسترجاع أو التضمينات |
| ٦ | اسألي سؤالًا لا يجيب عنه | «لم أجد» + ظهور فجوة معرفة | التحقق من المصادر |
| ٧ | أجيبي الفجوة من حساب مدير الشركة | تقول الرسالة «وصل التنبيه إلى ١ سائل» | فهرس التنبيهات أو تسجيل السائل |
| ٨ | ادخلي بحساب الموظف | التنبيه على الجرس وفيه نصّ الإجابة | التنبيهات |
| ٩ | افتحي كل صفحة في القائمة الجانبية | لا صفحة تسقط | مكوّنات الخادم |
| ١٠ | من حساب المالك: افتحي الشركات والخطط والدعم | تظهر البيانات، ومؤشرات الزوّار ليست «غير متاح» | صلاحيات الدوالّ |

**القاعدة:** لا يُدعى عميل قبل أن تمرّ العشر كلها في جولة واحدة متصلة.
والمرور جولةً واحدة أوثق من عشر تجارب متفرقة، لأن الأعطال تظهر في
الوصلات بين الخطوات لا في الخطوات نفسها.

---

## ٦) قبل أول عميل يدفع

| البند | لماذا |
|---|---|
| **Supabase Pro** | النسخة المجانية توقف قاعدة البيانات عند الخمول وبلا نسخ احتياطي. عميل يدفع لا يجوز أن يكون عليها. |
| **نطاق مخصّص** | `vercel.app` في العنوان يُضعف الثقة في عرض B2B. |
| **الفاتورة الضريبية ١٥٪** | إلزامية نظامًا. لم تُبنَ بعد. |
| **رفع PDF عربي** | أُصلحت ثلاث علل فيه ولم يُختبر على بيئتك. |
| **اختبارات العزل الـ١٦** | تحتاج قاعدة بيانات حيّة لتشغيلها. |

---

## ملحق — ترتيب ما يعتمد على ماذا

```
الترحيلات (١)
  ├── حدود الخطط تعمل
  ├── التنبيهات تصل
  └── جدول المدفوعات جاهز
        └── بوابة الدفع (٤)  ←  تحتاج مفاتيح Moyasar فقط
                                  (والسجل التجاري لاستلام المال، لا للاختبار)

روابط الدخول (٢)
  └── دعوة الموظفين واستعادة كلمة المرور تعملان
        └── توثيق النطاق (٣) يُخرجها من مجلد المزعج
```
