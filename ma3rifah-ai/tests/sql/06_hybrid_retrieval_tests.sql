...
-- =====================================================================
-- المجموعة ٧ — مقياس الرضا (0033)
-- =====================================================================
-- السبب نصٌّ يكتبه موظفٌ في شركةٍ عميلة، ويصل إلى مالك المنصّة. وهذا
-- تجاوزٌ مقصود لحدّ العزل — فيجب أن يكون **محدودًا ومحروسًا**: مالك
-- المنصّة وحده، وبلا سؤالٍ ولا إجابةٍ ولا اسم كاتب.
do $$
declare
  v_rows int;
  v_err  text;
begin
  -- مدير شركة يحاول قراءة المقياس
  perform set_config('request.jwt.claim.sub', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', false);
  begin
    perform * from public.platform_satisfaction(30);
    insert into public.test_results (category, name, passed, detail)
    values ('الرضا', 'مدير الشركة لا يقرأ مقياس الرضا', false, 'قرأه!');
  exception when insufficient_privilege then
    insert into public.test_results (category, name, passed, detail)
    values ('الرضا', 'مدير الشركة لا يقرأ مقياس الرضا', true, null);
  end;

  begin
    perform * from public.platform_feedback_notes(10);
    insert into public.test_results (category, name, passed, detail)
    values ('الرضا', 'مدير الشركة لا يقرأ أسباب غيره', false, 'قرأها!');
  exception when insufficient_privilege then
    insert into public.test_results (category, name, passed, detail)
    values ('الرضا', 'مدير الشركة لا يقرأ أسباب غيره', true, null);
  end;
  ...
  begin
    perform * from public.platform_satisfaction(30);
    insert into public.test_results (category, name, passed, detail)
    values ('الرضا', 'الزائر المجهول يُردّ', false, 'قرأه!');
  exception when others then
    insert into public.test_results (category, name, passed, detail)
    values ('الرضا', 'الزائر المجهول يُردّ', true, null);
  end;
end $$;

-- ضابط موجب: مالك المنصّة يقرأ
do $$
declare
  v_total bigint;
begin
  insert into auth.users (id, email) values
    ('99999999-9999-9999-9999-999999999999','owner@test.local')
  on conflict (id) do nothing;
  insert into public.profiles (id, company_id, email, full_name, role, status)
  values ('99999999-9999-9999-9999-999999999999',
          '11111111-1111-1111-1111-111111111111','owner@test.local','مالك','SUPER_ADMIN','ACTIVE')
  ...
  select total_rated into v_total from public.platform_satisfaction(30);

  insert into public.test_results (category, name, passed, detail)
  values ('الرضا', 'مالك المنصّة يقرأ المقياس (ضابط موجب)', v_total is not null,
          'مقيَّم=' || coalesce(v_total::text,'—'));
end $$;
